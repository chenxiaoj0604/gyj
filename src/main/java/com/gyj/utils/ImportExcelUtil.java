...
public class ImportExcelUtil {
    ...
    public static List<Map<String, Object>> parseExcel(InputStream in, String fileName,
                                                       Map<String, String> mapping,boolean isYL,int titleRowNum) throws Exception {
        // 根据文件名来创建Excel工作薄
        ...
        for (int i = 0; i < work.getNumberOfSheets(); i++) {
            ...
            log.info(new Gson().toJson(title));
            ...
            for (int j = titleRowNum + 1; j < sheet.getLastRowNum() + 1; j++) {
                ...
                if(row != null){
                    if(!isYL) {
                        String regex = "标准.*";
                        if (!String.valueOf(getCellValue(row.getCell(0))).matches(regex)) {
                            continue;
                        }
                    }
